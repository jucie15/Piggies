import os
import time
from piggyback.settings import CAST_ROOT, BASE_DIR
...
def contents_db_create(_keyword_list):
    # youtube 영상을 크롤링함과 동시에 디비에 저장

    driver = webdriver.Chrome(os.path.join(BASE_DIR, 'chromedriver'))  # 크롬 드라이버 사용

    if not _keyword_list:
        with open(CAST_ROOT('txt', 'crawling_list.txt'), 'rt') as f:
            keyword_list = f.read().split('\n')[:-1] # 파일에서 키워드 리스트를 받아온다
    else:
        keyword_list = _keyword_list
